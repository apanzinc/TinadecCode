...
public static class ProviderErrorMapper
{
    ...
    {
        return category is
            ProviderErrorCategory.RateLimited or ProviderErrorCategory.Timeout
            or ProviderErrorCategory.ProviderUnavailable;
    }

    private static ProviderFailureDetails Create(string providerId, ProviderErrorCategory category, int? statusCode,
        int? exitCode)
    {
        return new ProviderFailureDetails(category, IsRetryable(category), statusCode, exitCode, providerId,
