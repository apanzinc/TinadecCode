...
- `git_worktree_manager` retains read, index, and commit actions as compatibility adapters: they forward to approval-gated TinadecTools tools, so Gateway must not execute Git locally for them. `git_stage`, `git_unstage`, and `git_commit` require a Core-verified `kind=git` approval; commit also requires explicit `confirm_commit`. Push remains on the manager path; merge, rebase, branch creation, checkout, and worktree creation remain pending Tool-layer migration.
