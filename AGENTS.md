...
- Model Center composes Core-owned provider/catalog readiness receipts into a compact health overview, collapsible diagnostics, and a local searchable provider/template table; Desktop may filter and sort those receipts but must not reclassify readiness semantics.
